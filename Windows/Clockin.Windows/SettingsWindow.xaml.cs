...
public partial class SettingsWindow : Window
{
    ...
    public SettingsWindow(ClockStore store, ExchangeRateStore rates)
    {
        ...
        RateBox.Text = store.HourlyRate.ToString("0.##"); SelectItem(CurrencyBox, store.CurrencyCode); SelectItem(ThemeBox, _settings.Get("Theme", "Carbon")); SelectItem(PinnedModeBox, _settings.Get("PinnedMode", "Money")); SelectItem(MascotDefaultBox, _settings.Get("MascotDefault", "Auto"));
        ...
        PinEnabled.IsChecked = store.PinVisible; MascotEnabled.IsChecked = _settings.GetBool("MascotEnabled", true); MinimalMode.IsChecked = _settings.GetBool("MinimalMode"); MinimalShowHours.IsChecked = _settings.GetBool("MinimalShowHours", true); MinimalShowEarnings.IsChecked = _settings.GetBool("MinimalShowEarnings", true); MinimalShowTRY.IsChecked = _settings.GetBool("MinimalShowTRY", true); MinimalShowGoal.IsChecked = _settings.GetBool("MinimalShowGoal");
    ...
    private void Apply_Click(object sender, RoutedEventArgs e)
    {
        ...
        _store.UpdateRate(rate); _store.UpdateCurrency(Item(CurrencyBox, "USD")); _settings.Set("Theme", Item(ThemeBox, "Carbon")); _settings.Set("PinnedMode", Item(PinnedModeBox, "Money")); _settings.Set("MascotDefault", Item(MascotDefaultBox, "Auto")); _settings.Set("MascotEnabled", MascotEnabled.IsChecked == true); _settings.Set("GoalDailyHours", Parse(DailyGoal.Text)); _settings.Set("GoalMonthlyHours", Parse(MonthlyGoal.Text)); _settings.Set("MinimalMode", MinimalMode.IsChecked == true); _settings.Set("MinimalShowHours", MinimalShowHours.IsChecked == true); _settings.Set("MinimalShowEarnings", MinimalShowEarnings.IsChecked == true); _settings.Set("MinimalShowTRY", MinimalShowTRY.IsChecked == true); _settings.Set("MinimalShowGoal", MinimalShowGoal.IsChecked == true);
        var shouldPin = PinEnabled.IsChecked == true;
        if (MinimalMode.IsChecked == true) _settings.Set("PinVisibleBeforeMinimal", shouldPin);
        ...
        if (MinimalMode.IsChecked == true) { _store.SetPinned(false); Owner?.Hide(); }
    }
